...
namespace webclient {
	...
	bool HtmlParser::getHttpPage(LPCTSTR pszServerName, LPCTSTR pszFileName, int port, string& ret)
	{
	...
	bool HtmlParser::readHtml(const string& inHtml) {
		string html;
		regex pat("^http.*");
		if (regex_match(inHtml, pat)) {
			...
			if (!getHttpPage(strServer, strObj, nPort, html)) {
				cout << "サイトからの読み込みに失敗しました。\n";
				return false;
			}
		}
		else {
	...
	bool HtmlParser::outXml(const string& outfilename) {
